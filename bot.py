...
async def request(
    ...
    *raw_series_name: str,
    ...
    series_name = ""
    for item in raw_series_name:
        series_name = series_name + "%20" + item
...
async def approve(ctx, *approve_select):
    ...
    if ctx.author.id == admin_user.id:
        ...
        if approve_select and len(Pending_Requests) > 0:
            ...
            series = {
                key: value
                for key, value in series_raw.items()
            ...
            await ctx.send(
                "please select an option to approve from below:", embed=Pending_embed
            )
        else:
...
async def deny(ctx, *deny_select):
    ...
    if ctx.author.id == admin_user.id:
        ...
        if deny_select and len(Pending_Requests) > 0:
            ...
            await ctx.send(
                "please select an option to deny from below:", embed=Pending_embed
            )
        else:
            await ctx.send("no pending requests")
    else:
